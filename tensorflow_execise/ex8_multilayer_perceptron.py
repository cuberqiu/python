# A Multilayer Perception implementation example using TensorFlow library
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            # Run optimization op (backpropgation) and cost op(to get loss value)
            _,c = sess.run([optimizer,cost],feed_dict={x:batch_x,y:batch_y})

            # Compute average loss
            avg_cost += c/total_batch

        # Display per epoch step
        if epoch % display_step == 0:
            print "Epoch:",'%04d'%(epoch+1),"cost=","{:.9f}".format(avg_cost)
    print "Optimization Finished!"

    # Test mdoel
    correct_prediction = tf.equal(tf.argmax(ped,1),tf.argmax(y,1))
    # Calculate accuracy
